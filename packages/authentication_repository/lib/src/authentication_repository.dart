...
class AuthUserChangeFailure implements Exception {
  /// {@macro sign_up_with_email_and_password_failure}
  const AuthUserChangeFailure([
    this.message = 'An unknown exception occurred.',
  ]);
...
class AuthenticationRepository {
  ...
  Stream<User> get user => _user.asBroadcastStream();

  /// Gets the current user synchronously.
  User get currentUser => _user.valueOrNull ?? User.empty;
  ...
  Future<void> signUp({required String email, required String password}) async {
    try {
      ...
      unawaited(_updateUserData(result.user));
  ...
  /// Marks a badge as completed.
  ///
  /// It also increments the total no of completed quizzes.
  Future<void> markBadge() {
    return _firestore.collection('users').doc(currentUser.id).set(
      <String, dynamic>{
        'badge': true,
      },
  ...
  ///
  /// It also increments the total no of completed content.
  Future<void> markContentComplete({
    ...
    return _firestore.collection('users').doc(currentUser.id).set(
      <String, dynamic>{
        'completedContent': {
          topicId: FieldValue.arrayUnion(<String>[contentId]),
        },
      },
      SetOptions(merge: true),
    );
  ...
  Future<void> _updateUserData(firebase_auth.User? firebaseUser) async {
    ...
    final userJson = user.toJson()..removeWhere((key, value) => value == null);
...
extension _FirebaseAuthExtensions on firebase_auth.FirebaseAuth {
  ValueStream<User> authUserChanges(FirebaseFirestore firestore) =>
      authStateChanges()
          ...
          .handleError((Object _) => throw const AuthUserChangeFailure())
          .shareValue();
}
...
extension StreamExtensions<T> on Stream<T> {
  ...
  Stream<T> handleFailure<F extends Exception>([
